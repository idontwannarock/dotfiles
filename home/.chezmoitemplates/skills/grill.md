...
- 一次一題,那麼是哪一題 — 由依賴決定。某題的答案取決於另一題時,
  把 `blocked by` 標下來,先問沒有被擋住的題。
- 每題附上你的建議答案,讓使用者可以只回「好」。
- 事實 vs 決策:能從環境查到的事實(程式碼、文件、git 歷史、
  過去的 spec)自己查,不要拿去問;決策永遠屬於使用者。
- 訪談開場先讀 `context/`(若存在)當 domain grounding:
...
- 長青候選(疑似跨 change 反覆適用的 domain 詞彙 / 原則)→ 以
  ...
  grill **不寫** `context/`;是否晉升由 sync/archive 階段
  對照實際實作決定(見 dev-workflow)。
- 尚未釐清(訪談中浮現、但還收斂不成一個精確問題的不確定性)→ 記下來,
  不要讓它在共識確認的那一刻靜靜消失。確認共識時一併呈出,每項請使用者
  三選一:接受風險往下走 / 現在再問 / 移出這次範圍。裁決後仍留著的,
  記入 design.md 的 `## Open Questions`。

  這一節**不構成 gate** — 唯一的 gate 仍是共識確認,已知的未知永遠不會
  歸零。沒有這種項目時完全不要提它,不要為了填欄位硬擠。
