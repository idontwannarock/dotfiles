...
- **Retiring anything a script installed** follows the same rule as `.chezmoiremove`, one level up: deleting the *install* line only stops new machines from getting it. Machines that already applied keep it forever. The script must actively remove it. Claude Code plugins have a `$retiredPlugins` / `$RETIRED_PLUGINS` table in `run_onchange_install-03-claude-config.{ps1,sh}.tmpl` — retiring a plugin means **adding a row there**, not deleting the install line. Keep the two interpreters' tables identical. Removal converges only once every machine has applied.
