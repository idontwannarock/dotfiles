...
# the swap is destructive (no pwsh between remove and install), and pwsh is chezmoi's
# .ps1 interpreter (chicken-and-egg). So this is a deliberate manual, one-shot helper.
# Keep a Windows PowerShell 5.1 window open as a fallback while this runs.
...
# 2. Remove the Store/MSIX PowerShell (per-user package + provisioned package).
Write-Host "[1/4] Removing Store/MSIX PowerShell (per-user + provisioned)..." -ForegroundColor Cyan
...
# 3. Resolve + download the latest STABLE MSI from GitHub (releases/latest excludes prereleases).
Write-Host "[2/4] Resolving latest stable PowerShell MSI from GitHub..." -ForegroundColor Cyan
...
# 4. Verify Authenticode (Valid + signed by Microsoft) BEFORE running the installer.
...
Write-Host "      Authenticode OK: $($sig.SignerCertificate.Subject)" -ForegroundColor Gray

# 5. Install silently.
Write-Host "[3/4] Installing MSI (msiexec /qn)..." -ForegroundColor Cyan
