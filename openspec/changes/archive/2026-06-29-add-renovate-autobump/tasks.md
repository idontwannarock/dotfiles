...
- [x] 6.1 Write root `renovate.json`: `extends` base config, custom regex manager with `managerFilePatterns` for `home/.chezmoiexternal.toml` and the matchString capturing the `# renovate:` comment + the `{{- $…Version := "…" }}` line
- [x] 6.2 Set PR hygiene: `dependencyDashboard: true`, weekly `schedule`, labels, no `automerge`, `ignoreUnstable`

## 7. Verify

- [x] 7.1 Render the annotated file again and diff against the "before" baseline — non-JDK lines byte-identical; JDK lines render the same URLs
- [x] 7.2 Run `npx --yes renovate-config-validator` and confirm valid
- [x] 7.3 Dry-run Renovate (or inspect the would-be dependency dashboard) and reconcile the detected-dependency count against the Tier A + Tier B + JDK set; fix any missed/mis-captured pin
- [x] 7.4 Confirm no deferred tool (jdtls/ffmpeg/dos2unix/vim) and neither `statusline`/`passgen` appears in the detected set
...
- [x] 8.2 Note in repo docs how to add a future tool (two-line `# renovate:` comment) and that jdtls/ffmpeg/dos2unix/vim are intentionally deferred to the mirror phase
