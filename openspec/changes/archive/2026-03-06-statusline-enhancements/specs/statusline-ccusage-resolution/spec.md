...
- **THEN** ccusage-dependent features (today cost) are skipped; block timer uses JSONL instead
...
- **WHEN** ccusage returns empty output and a cached cost file exists
- **THEN** the cache file retains previous value and the display shows the cached value

#### Scenario: ccusage fails with no prior cache
- **WHEN** ccusage returns empty output and no cache file exists
- **THEN** no cache file is created and the today cost section is omitted
