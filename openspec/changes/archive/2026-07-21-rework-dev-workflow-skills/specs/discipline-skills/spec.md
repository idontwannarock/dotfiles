...
- **THEN** `worktree` SHALL 以 `git --git-dir=.bare worktree add -b <branch> <dir> main` 建立並登記 active_workflows

#### Scenario: bare-worktree 下收尾
- **WHEN** ARCH=bare-worktree 且執行 `finish-branch`
- **THEN** SHALL rebase 後從 `main/` worktree 以 `--ff-only` merge,並處置 worktree 與 branch,全程不需查閱額外 override 文件
