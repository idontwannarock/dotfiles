...
- **兩平台訊息逐字一致由測試釘住。** 起初打算沿用「bash 端沒有測試框架,靠 code
  review 比對」的既有作法,但那個理由站不住:要比對的是**兩份原始檔裡的字串**,不是
  執行 bash,所以它在既有的 Pester 套件裡跑得起來。測試從兩份原始碼各抽出訊息字面值、
  把 `%s` 正規化成 `{0}` 後逐字比對,並以突變測試確認它會轉紅(改動 bash 端一個字,
  該測試由綠轉紅)。
