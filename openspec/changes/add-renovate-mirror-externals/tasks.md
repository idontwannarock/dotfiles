...
- [ ] 2.2 Verify: chezmoi render-diff shows only the intended ffmpeg change; after `chezmoi apply`, `ffmpeg -version` and `ffprobe -version` work
...
- [ ] 4.1 Push the branch and run the workflow via `workflow_dispatch`; confirm three releases `mirror-{vim,jdtls,dos2unix}-<current-pin>` are created at the currently-pinned versions
- [ ] 4.2 Verify each seeded release's notes record the upstream source URL, upstream version, and upstream-file sha256

## 5. Repoint the mirrored externals and decouple vim wrappers

- [ ] 5.1 jdtls: point the external URL at `mirror-jdtls-<v>` (`jdt-language-server-<v>.tar.gz`), reword the `# renovate: ignore` comment to "mirrored by mirror-externals.yml"
- [ ] 5.2 dos2unix: change `type = "archive-file"` → `type = "file"`, point the URL at `mirror-dos2unix-<v>` (`dos2unix-<v>.exe`), reword the ignore comment
- [ ] 5.3 vim: point the external URL at `mirror-vim-<v>.zip`, drop `stripComponents` (mirror zip is rooted at `current/`), keep `type = "archive"` so it extracts to `.local/share/vim/current/…`, reword the ignore comment
- [ ] 5.4 Edit the 15 `.cmd` wrappers in `home/dot_local/bin/` (`vim vi view ex evim eview rvim rview gvim gview gvimdiff rgvim rgview vimdiff xxd`): replace `vim92` with `current` and add `set "VIMRUNTIME=%USERPROFILE%\.local\share\vim\current"`
- [ ] 5.5 Confirm no remaining `vim92` reference exists outside archived openspec changes

## 6. Verification

- [ ] 6.1 chezmoi render-diff (before/after) of `home/.chezmoiexternal.toml`: every effective change is intended (ffmpeg → GyanD; the three URLs → mirror releases; deployed binaries equivalent)
...
- [ ] 7.1 Update `docs/renovate.md`: move ffmpeg into the tracked tiers (GyanD); move vim/jdtls/dos2unix out of the "Intentionally not tracked" table into a documented mirror-phase section describing the workflow, tag scheme, and the two bump-PR streams
