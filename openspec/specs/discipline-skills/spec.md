...
#### Scenario: 範圍標記不得宣稱檔案＋通知格完全不經 shell
- **WHEN** 標記說明哪些內容不再適用吃字機制
- **THEN** SHALL 把**檔案＋通知格的通知句**留在適用範圍內——它有一到兩行說明與一個路徑,照樣經過 shell
- **AND** SHALL 寫明升級規則救不了它:**它已經在檔案＋通知格,沒有更上面可去**
...
`coordinate` SHALL 規定艦隊產物(名冊、map 與**線間訊息**)落在 `~/.agent/fleets/<repo-slug>/<fleet>/`,SHALL NOT 與 `active_workflows.md` 混放於同一目錄。`<repo-slug>` SHALL 明確選定一個 slugify 派別並寫出理由,SHALL NOT 寫成「與 handoff、workflow 狀態同一個 key」——`repo-identity.md` 明文記載兩派尚未收斂,新機制 SHALL NOT 假設兩派產生同一個 key。選擇 SHALL 是 handoff 家族(`:` `\` `/` `.` 皆換為 `-`),理由 SHALL 是搬移目的地正是 handoff 的 `attachments/`。線間訊息 SHALL 落在該目錄下的 `msgs/` 子目錄。
