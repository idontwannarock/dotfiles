# tools-build-check Specification

## Purpose
規範 `tools/` 下可編譯原始碼（`tools/statusline/` 的 Go、`tools/passgen/` 的 Rust）在 PR 階段的建置驗證契約：PR-only 觸發、Go 端須涵蓋 release 產出的全部四個平台組合（因 build tag 分流使單平台檢查看不到 `count_windows.go`）、Rust 端執行既有測試且在無平台條件編譯的前提下豁免跨編譯。此 workflow 純檢查，不產出 artifact。
