# okf-bundle-conventions Specification

## Purpose
定義本 repo 各 Open Knowledge Format (OKF) v0.2 bundle 共用的格式規則:concept 檔的 frontmatter 欄位子集、`type` 詞彙與其擴充閘門、`index.md` 的 reserved 地位與內容邊界、bundle root 的 `okf_version` 宣告,以及適用範圍守衛。內容規則不在此 —— 各 bundle 由自己的 capability 規範裝什麼。
