...
## Purpose
定義 repo-level 的長青專案 context:它是 repo root 的 `context/` OKF bundle,承載 domain 背景、詞彙與反覆適用的原則,供需求分析時查閱而非每 session 自動載入。規範其位置理由、依知識性質分檔的規則、三分法內容邊界與晉升閘門。格式規則見 `okf-bundle-conventions`。
