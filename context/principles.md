...
- **一段結論該落在哪份載體,問的是「讀者在哪」。** 四格排在同一條軸上:`specs/` = WHAT(行為契約)、`design.md` = 一次性決策、`context/` = 可重用的原則與 domain 詞彙 —— 這三格的讀者都在 repo 內(agent 做需求分析時讀);第四格是團隊 wiki,讀者在 repo 外。可操作化的問法是「repo 外的人要回答這個問題,除了讀這份 diff 之外有沒有別的地方可讀」,答「沒有」就屬第四格。這條同時給出**該不該寫團隊文件**的判準,而它綁的是讀者、不是 diff 大小 —— 改一行連線參數卻產出別隊要照做的切換程序,是典型的小 change 配 RUNBOOK;反過來重構兩千行內部腳本,一格都不占。 <!-- positional-ref: 講的是文件載體四格（specs/、design.md、context/、docs/），與訊息載體同字不同族 -->
...
- **序號不得承重。** 條文裡指涉「第五項」「第六項」這種位置,會在清單插入一項的當下同時變假,而且**沒有任何東西會轉紅**。指涉一律以內容命名(「編號回收」而非「第五項」);序號只用於呈現順序。 <!-- positional-ref: 規則自述，引「第五項」「第六項」作為反例，是 mention 不是 use -->
