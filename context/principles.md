...
- **agent 之間傳一則內容,先選載體,而載體有三種不是兩種。** 判準是「這則內容要在對方的**哪個時刻**生效」:只在現在 → 訊息;在未來某個時刻 → 檔案,訊息只說有一份東西在那裡;**現在要知道且未來要能重讀 → 兩個都要**。第三格最常被漏掉,而它的代價是**延遲的**——內容當下確實送到了,等到有人要重驗它時只剩被壓縮過的摘要,且查不出漏了什麼。推論是**判準要能機械地把內容推向正本**:含高密度技術符號(那些字元會被傳輸層靜默吃掉)或長到需要分段的內容一律歸第三格,因為那正是事後最需要原文的同一批內容——守衛的方向與判準自洽,不是外加的檢查。這與「資料通道不得是螢幕輸出」同族,是它在「訊息 vs 檔案」這一軸上的延伸。
