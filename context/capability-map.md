...
**chezmoi 骨架** — 定義 source 樹的形狀與渲染規則:哪些檔案部署到哪、模板如何切分、檔名前綴的語意。判準是「這條規則描述 chezmoi 怎麼把 source 變成機器上的產物」。
...
**statusline** — 本 repo 唯一的自有可執行產物(Go binary)及其發佈與資料來源。判準是「這是我們寫的程式碼,不是設定」。

**開發流程** — spec/change 生命週期、紀律 skills、workflow 併發與收尾、工作記錄觸發、架構體檢。判準是「這條規則規範的是**怎麼做事**,不是機器上有什麼」。

**知識載體** — 跨 tool 共用 reference 的配置、OKF bundle 的格式規則、repo-level 專案 context。判準是「這規範的是知識放哪、長什麼樣,而非知識本身」。內容本身不在 `specs/`:machine-level 的在 `~/.agent/reference/`,repo-level 的就是你正在讀的 `context/`。
