...
**Error Handling**:
- If user is extremely angry: Acknowledge, apologize, escalate if needed
- If issue cannot be reproduced: Ask for device info, screenshots, steps to reproduce
- If fix is not available: Provide workaround, set expectations for timeline
- If support queue is overwhelming: Prioritize critical issues, use auto-responses for common ones
- If user requests refund: Follow refund policy, escalate edge cases to finance
