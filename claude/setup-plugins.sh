...
# 3. 複製全域 CLAUDE.md
echo ""
echo "[3/$total_steps] Installing global CLAUDE.md..."
...
# 4. 複製 ensure-openspec.sh 到 ~/.local/bin/
echo ""
echo "[4/$total_steps] Installing ensure-openspec.sh to ~/.local/bin/..."
...
echo ""
echo "[5/$total_steps] Installing /ensure-openspec skill..."
...
echo ""
echo "[6/$total_steps] Installing /opsx commands..."
...
echo ""
echo "[7/$total_steps] Installing /sp commands (superpowers shortcuts)..."
sp_src="$repo_dir/.claude/commands/sp"
sp_dest="$HOME/.claude/commands/sp"
mkdir -p "$sp_dest"
cp "$sp_src"/*.md "$sp_dest/"
echo "  Installed $(ls "$sp_dest"/*.md 2>/dev/null | wc -l) commands."
echo "  Done."

# 8. 複製 git commands 到 ~/.claude/commands/git/
echo ""
echo "[8/$total_steps] Installing /git commands (git shortcuts)..."
git_src="$repo_dir/.claude/commands/git"
git_dest="$HOME/.claude/commands/git"
mkdir -p "$git_dest"
cp "$git_src"/*.md "$git_dest/"
